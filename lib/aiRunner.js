 ...
 */
import { isPuterModel } from './puterAi.js';
...
/**
 * Fleet LLM waterfall (cheap by default — no Cohere, no Puter Fable/Astra):
 *  1) Gemini BYOK
 *  2) Groq
 *  3) Workers AI binding cascade
 *  4) Workers AI REST
 * Puter Fable/Astra is opt-in in handleChat, not this default hop.
 */
export async function runWorkersAi(env, model, body = {}, roleConfig = {}, fullMessages) {
  const useModel = normalizeModel(model, env);
  const errors = [];

  if (isPuterModel(useModel) || body.prefer === 'puter' || body.prefer === 'cohere') {
    throw new Error('paid/removed providers are not in the cheap waterfall');
  }
