 ...
 */
import { isCohereConfigured, runCohereChat } from './cohereDedicated.js';
...
/**
 * Best-practice Grudge LLM waterfall:
 *  0) Cohere dedicated (grudge-s010rs) when COHERE_API_KEY set
 *  1) Gemini BYOK (google/* when key set)
 *  2) Groq free (GROQ_API_KEY)
 *  3) Workers AI binding cascade
 *  4) Workers AI REST
 */
export async function runWorkersAi(env, model, body = {}, roleConfig = {}, fullMessages) {
  ...
  // 0) Cohere dedicated cluster
  if (isCohereConfigured(env) && body.prefer !== 'gemini' && body.prefer !== 'groq' && body.prefer !== 'workers') {
    try {
      const msgs =
        fullMessages ||
        body.messages ||
      ...
      if (msgs?.length) {
        const c = await runCohereChat(env, msgs, String(useModel).includes('command') ? useModel : undefined);
        if (c?.text) {
          return { ...c, fallback: false, path: ['cohere-dedicated'] };
        }
      }
    } catch (cohErr) {
      errors.push(`cohere: ${cohErr.message}`);
      console.warn('Cohere dedicated failed:', cohErr.message);
    }
  }
