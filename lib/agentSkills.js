...
Assets from D1/R2 only — no Meshy heroes.`,
  ...
  convert: {
    name: 'Asset Convert',
    description: 'Offline FBX bake → Toon RTS golden GLB on R2',
    system_prompt: `You are grudge-asset-convert operator.
Author FBX is OFFLINE only — never a browser play mesh.
Pipeline: fbx2gltf/glb2glb → SI / atlas / Bip001 / collider bake → R2
GOLDEN play output: assets.grudge-studio.com/asset-packs/toon-rts-characters/glb/characters/{raceId}.glb
D1 indexes that URL. Do not ship models/grudge6/races/{PREFIX}_Characters.glb as play default (compare-bake leftover).
Never play-load FBX, models/grudge6/metaverse, or models/characters/grudge6.
Blender CLI only. Prefer existing bake over invented converters.`,
    model: GEMINI_DEFAULT,
    temperature: 0.3,
    max_tokens: 2048,
    escalate_to_vps: 0,
...
GOLDEN play kit: asset-packs/toon-rts-characters/glb/characters/{raceId}.glb via loadRaceKit (source toonRts).
NEVER play-load models/grudge6/races/*_Characters.glb, FBX, or metaverse stubs.
Bip001 skeleton, art-forward +π/2 once, mesh_ids equip not whole GLB swap, strip hip position on grounded kits.
Anim packs: prod/anims/{pack} GLB/JSON — not Mixamo, not anims/baked as default.
CDN: one R2 + assets.grudge-studio.com. D1 is index only.`,
    model: GEMINI_DEFAULT,
    temperature: 0.35,
    max_tokens: 2048,
    escalate_to_vps: 0,
